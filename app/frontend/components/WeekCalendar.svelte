<script lang="ts">
  ...
  function getEntriesForDay(date: Date) {
    ...
    return entries.filter((entry: { start_at: string; end_at: string | null }) => {
      const entryStart = new Date(entry.start_at)
      return entryStart >= startOfDay && entryStart <= endOfDay
    })
  }

  // Calculate entry position and height
  function getEntryStyle(entry: { start_at: string; end_at: string | null; duration_seconds: number | null }) {
  ...
  function getDayTotal(date: Date) {
    ...
    let totalSeconds = 0
    dayEntries.forEach((entry: { duration_seconds: number | null; end_at: string | null; start_at: string }) => {
  ...
  function handleMouseDown(day: Date, event: MouseEvent) {
    ...
    const minute = Math.floor((minutes % 60) / 15) * 15 // Round to 15 min
  ...
  function handleMouseMove(event: MouseEvent) {
    ...
    const dayColumn = document.querySelector(`[data-day="${dragDay.toISOString()}"]`)
    if (!dayColumn) return
    ...
    dragCurrent = { hour: Math.max(startHour, Math.min(endHour, hour)), minute }
  }
...
<div class="flex flex-col h-full">
  <!-- Header -->
  <div class="flex items-center justify-between p-4 border-b border-bg-tertiary bg-bg-secondary">
  ...
  <div class="flex-1 overflow-auto">
    <div class="min-w-[800px] p-4">
      ...
      <div class="grid grid-cols-8 gap-1 mb-2">
        <div class="w-16"></div> <!-- Time column spacer -->
        {#each days as day}
      ...
      <div class="grid grid-cols-8 gap-1 relative">
        ...
        <div class="w-16 flex flex-col">
          {#each Array.from({ length: endHour - startHour + 1 }, (_, i) => startHour + i) as hour}
            <div class="text-xs text-fg-muted text-right pr-2" style="height: {hourHeight}px; line-height: {hourHeight}px;">
              {formatHour(hour)}
            </div>
          {/each}
        </div>
        ...
        {#each days as day}
          ...
          >
            ...
            {#each Array.from({ length: endHour - startHour }, (_, i) => i) as _}
              <div class="border-b border-bg-tertiary/30" style="height: {hourHeight}px;"></div>
            {/each}

            <!-- Time Entries -->
            {#each getEntriesForDay(day) as entry}
              <div 
                class="absolute left-1 right-1 rounded-[6px] px-2 py-1 text-xs overflow-hidden cursor-pointer hover:brightness-110 transition-all duration-200"
                style="{getEntryStyle(entry)} background-color: {entry.project?.color || '#b16286'}40; border-left: 3px solid {entry.project?.color || '#b16286'};"
                title="{entry.description || 'No description'} ({entry.formattedDuration})"
              >
                <div class="font-medium truncate text-fg-primary">{entry.description || 'No description'}</div>
                <div class="text-fg-secondary truncate">{entry.formattedDuration}</div>
              </div>
            {/each}
            ...
            {#if isDragging && dragDay?.toDateString() === day.toDateString()}
              <div 
                class="absolute left-1 right-1 rounded-[6px] bg-bright-blue/30 border-2 border-bright-blue border-dashed pointer-events-none"
                style="{getDragStyle()}"
              ></div>
            {/if}
          </div>
...
{#if showQuickEdit}
  ...
  >
    ...
    >
      <h3 id="quick-edit-title" class="text-xl font-semibold">New Time Entry</h3>
      <p class="text-fg-muted text-sm">{formatTimeRange()}</p>

      <div>
        <label for="entry-description" class="block text-sm font-medium mb-2">Description</label>
        <input
        ...
        <div class="grid grid-cols-4 gap-2 max-h-32 overflow-y-auto">
          {#each projects as project}
            <button
              ...
              class="p-2 rounded-[8px] border text-xs text-left transition-colors duration-150 {$form.project_id === project.id ? 'border-bright-blue bg-bright-blue/20' : 'border-bg-tertiary hover:border-fg-muted'}"
            >
              <span class="block w-3 h-3 rounded-full mb-1" style="background-color: {project.color}"></span>
      ...
      <div class="flex items-center justify-between">
        <button
          ...
          class="flex items-center gap-2 px-4 py-2 rounded-[10px] border transition-colors duration-150 {$form.billable ? 'bg-bright-green/20 border-bright-green text-bright-green' : 'border-bg-tertiary text-fg-muted'}"
        >
          <span>Billable</span>
        </button>
